"""
scan_local.py  —  Analyse log files in the CURRENT FOLDER
==========================================================

Use when logs are already on your machine and you want to
review specific files without FTP or a time range filter.

How it works:
  - Reads ALL files in the current directory (or subfolders you specify)
  - No date filter — every file is scanned
  - Same keyword search, 90% grouping, CSV, HTML, Case KB as scan_count.py
  - API key optional: leave blank for dummy analysis (function test mode)

Setup:
  1. Put scan_local.py in the folder that contains your log files
     (or set LOG_FOLDER below to point to the log folder)
  2. Make sure strings.txt is in the same folder
  3. Run:  python scan_local.py

Folder layout example:
    scan_local.py
    strings.txt
    cases.json          ← optional, auto-created on first save
    log2026051301       ← your log files (any name, any extension)
    log2026051302
    debug_pm03.log
    event_20260513.txt
    ...
...
CASES_FILE   = "cases.json"    # case knowledge base

# Anthropic API Key
# Get yours at https://console.anthropic.com/ → API Keys
# Leave blank to use dummy analysis (for testing without an API key)
ANTHROPIC_API_KEY = ""
...
# Change to a specific path if your logs are elsewhere, e.g.:
#   LOG_FOLDER = r"C:\Logs\PM03"
#   LOG_FOLDER = "/home/user/logs/260421"
LOG_FOLDER = "."

# ── FILE FILTER ───────────────────────────────────────────────────────────────
# Only scan files whose names contain ANY of these strings.
# Leave empty [] to scan ALL files in LOG_FOLDER.
# Examples:
#   FILE_FILTER = [".log"]            → only .log files
#   FILE_FILTER = ["260513", "260514"] → files with these dates in the name
#   FILE_FILTER = ["PM03", "PM05"]    → files with PM03 or PM05 in the name
FILE_FILTER = []

# ── SKIP THESE FILES ──────────────────────────────────────────────────────────
# Files to always skip (script files, outputs, etc.)
SKIP_NAMES = {
    "scan_local.py", "scan_count.py", "scan_count_test.py",
    "strings.txt", "cases.json", "requirements.txt",
}
SKIP_EXTENSIONS = {".py", ".csv", ".html", ".json", ".xlsx", ".docx", ".pdf"}

# ─────────────────────────────────────────────────────────────────────────────
current_time = datetime.now().strftime("%y%m%d-%H%M%S")
OUTPUT_CSV   = f"LocalScan_output_{current_time}.csv"
...
# ══════════════════════════════════════════════════════════════════════════════
# CASE KNOWLEDGE BASE  (identical to scan_count.py)
# ══════════════════════════════════════════════════════════════════════════════
def load_cases() -> list:
    if not os.path.exists(CASES_FILE): return []
    try:
        with open(CASES_FILE, "r", encoding="utf-8") as f: return json.load(f)
...
def add_case(title, tags, reproduction, log_patterns, analysis) -> dict:
    ...
    case  = {"id": str(uuid.uuid4())[:8], "title": title, "tags": tags,
             "reproduction": reproduction, "log_patterns": log_patterns,
             "analysis": analysis, "created_at": now_s, "updated_at": now_s}
    cases.append(case); save_cases(cases); return case

def find_similar_cases(issue_text, top_n=MAX_SIMILAR_CASES) -> list:
    cases = load_cases()
    if not cases: return []
    il = issue_text.lower(); scored = []
    for c in cases:
        score  = sum(10 for p in c.get("log_patterns",[]) if p.lower() in il)
        score += sum(5  for t in c.get("tags",[])         if t.lower() in il)
        score += sum(2  for w in c.get("title","").lower().split()
                     if len(w) > 3 and w in il)
...
def should_scan(fname: str) -> bool:
    """Return True if this file should be scanned."""
    # Skip by name
    if fname in SKIP_NAMES: return False
    # Skip by extension
    _, ext = os.path.splitext(fname)
    if ext.lower() in SKIP_EXTENSIONS: return False
    # Apply user file filter (if any)
    if FILE_FILTER:
        return any(f.lower() in fname.lower() for f in FILE_FILTER)
    return True


# ══════════════════════════════════════════════════════════════════════════════
# KEYWORD SUMMARY  (identical to scan_count.py)
# ══════════════════════════════════════════════════════════════════════════════
def print_keyword_summary(keyword_hits: dict, keyword_computers: dict):
    print(f"\n{'═'*65}")
    print(f"{Fore.CYAN}{Style.BRIGHT}  KEYWORD SUMMARY")
    print(f"{'═'*65}")
    print(f"  {'Keyword':<35} {'Hits':>6}  {'Files':>5}  Bar")
    print(f"  {'─'*35}  {'─'*6}  {'─'*5}  {'─'*15}")
    if not keyword_hits: print(f"  {Fore.MAGENTA}No hits."); print(f"{'═'*65}"); return
    ...
    for kw, count in sorted(keyword_hits.items(), key=lambda x: -x[1]):
        ...
        files = len(keyword_computers.get(kw, set()))
        color = Fore.RED if count > max_h*0.6 else Fore.YELLOW if count > max_h*0.2 else Fore.GREEN
        kd    = (kw[:33]+"..") if len(kw) > 35 else kw
        print(f"  {Fore.WHITE}{kd:<35} {color}{count:>6}  "
              f"{Fore.CYAN}{files:>5}  {color}{bar}{Style.RESET_ALL}")
    print(f"{'═'*65}\n")


# ══════════════════════════════════════════════════════════════════════════════
# ANTHROPIC API  (identical to scan_count.py)
# ══════════════════════════════════════════════════════════════════════════════
def call_claude_api(system_prompt: str, user_message: str) -> str:
    if not ANTHROPIC_API_KEY:
        return (
            "[ERROR] ANTHROPIC_API_KEY is not set.\n"
            "Paste your key (sk-ant-...) into ANTHROPIC_API_KEY at the top of this file."
        )
    url     = "https://api.anthropic.com/v1/messages"
    headers = {"Content-Type": "application/json",
               "x-api-key": ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01"}
    payload = {"model": ANTHROPIC_MODEL, "max_tokens": 2048,
...
# ══════════════════════════════════════════════════════════════════════════════
# AI ANALYSIS  (identical to scan_count.py)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# SAVE CASE  (identical to scan_count.py)
# ══════════════════════════════════════════════════════════════════════════════
def prompt_save_case(entry: dict, analysis_text: str):
    ...
    print(f"\n{Fore.GREEN}✓ Case saved — ID: {case['id']}  ({CASES_FILE})")


# ══════════════════════════════════════════════════════════════════════════════
# HTML REPORT  (identical to scan_count.py)
# ══════════════════════════════════════════════════════════════════════════════
...
<title>Local Scan — {_e(scan_range_from)}</title>
<style>
...
.header .range{{font-size:11px;color:var(--muted);margin-top:2px;}}
...
<aside class="sidebar">
  <div class="header">
    <h1>Local Scan Report</h1>
    <div class="range">📁 {_e(os.path.abspath(LOG_FOLDER))}</div>
...
function selectIssue(id){{
  ...
  const cBanner=relC.length?`<div style="margin:12px 0;padding:10px 13px;background:var(--purple-bg);border-radius:7px;border:1px solid rgba(124,58,237,.15);">
    ...
    ${{relC.map(c=>`<div style="font-size:12px;cursor:pointer;" onclick="switchTab('cases');setTimeout(()=>selectCase('${{c.id}}'),50)">→ ${{escH(c.title)}}</div>`).join('')}}</div>`:'';
  document.getElementById('main-panel').innerHTML=`
    ...
    <div class="card"><div class="card-header"><span>AI Analysis</span>
      ${{hasAI?`<button class="save-btn" onclick="openSaveCaseModal(${{is.id}})">📂 Save as Case</button>`:''}}
    </div><div class="card-body">${{aiHtml}}</div></div>`;
}}
function renderKwList(items){{document.getElementById('kw-list').innerHTML=items.map((kw,i)=>`
  <div class="kw-item" id="kwrow-${{i}}" onclick="selectKeyword(${{i}})">
    ...
    <div class="kw-meta"><span>Hits:<strong>${{kw.hits}}</strong></span><span>Files:<strong>${{kw.computers}}</strong></span></div>
  </div>`).join('');
}}
  ...
  document.getElementById('main-panel').innerHTML=`
    <div class="detail-header"><h2 style="font-family:monospace">"${{escH(kw.kw)}}"</h2>
      <div class="meta-row"><span class="badge kw">keyword</span>
        ...
        <span class="meta-chip">📄 ${{kw.computers}} file${{kw.computers!==1?'s':''}}</span></div></div>
...
function openSaveCaseModal(issueId){{
  ...
  document.getElementById('modal-delete-btn').style.display='none';document.getElementById('modal').classList.add('open');
}}
...
function openEditCaseModal(id){{
  ...
  document.getElementById('modal-title').textContent='Edit Case';document.getElementById('modal-case-id').value=c.id;
  ...
  document.getElementById('modal-delete-btn').style.display='inline-block';document.getElementById('modal').classList.add('open');
}}
...
# ══════════════════════════════════════════════════════════════════════════════
# CSV UPDATE  (identical to scan_count.py)
# ══════════════════════════════════════════════════════════════════════════════
def update_csv_with_analysis(csv_path, analyses):
    ...
    with open(csv_path, "r", encoding="utf-8-sig") as f:
        reader = csv.DictReader(f); fns = list(reader.fieldnames or [])
        if "AI_Analysis" not in fns: fns.append("AI_Analysis")
        for row in reader:
            key = (row.get("ComputerName",""), row.get("Error",""))
            row["AI_Analysis"] = amap.get(key, row.get("AI_Analysis","")); rows.append(row)
    with open(csv_path, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=fns); writer.writeheader(); writer.writerows(rows)
    print(f"{Fore.GREEN}CSV updated: {csv_path}")


# ══════════════════════════════════════════════════════════════════════════════
# INTERACTIVE ANALYSIS  (identical to scan_count.py)
# ══════════════════════════════════════════════════════════════════════════════
def interactive_analysis(all_summaries, keyword_hits, keyword_computers,
                         scanned_at: str, folder_path: str):
    if not all_summaries: print(f"\n{Fore.MAGENTA}No issues to analyse."); return
    all_analyses = []; csv_analysis_map = {}

    def print_issue_list():
        cases = load_cases()
        print(f"\n{'='*70}")
        print(f"{Fore.CYAN}{Style.BRIGHT}  [ AI ISSUE ANALYSIS MODE ]")
        print(f"{Fore.WHITE}  Folder : {folder_path}")
        ...
        for i, item in enumerate(all_summaries, 1):
            ...
            print(f"  {Fore.YELLOW}[{i:>3}] {done}{Fore.WHITE}"
                  f"{e['ComputerName']:<22} {Fore.CYAN}×{e['Count']:<5}"
                  f"{Fore.WHITE}{prev}{hint}{Style.RESET_ALL}")
        print(f"\n{Fore.WHITE}number(s) / {Fore.YELLOW}all{Fore.WHITE} / "
              f"{Fore.YELLOW}report{Fore.WHITE} / {Fore.YELLOW}q")
    ...
    while True:
        ...
        if raw == "report":
            _flush_report(all_summaries, all_analyses, csv_analysis_map,
                          keyword_hits, keyword_computers, scanned_at, folder_path); continue
        if raw == "all": selected_indices = list(range(len(all_summaries)))
        else:
            try:
            ...
            except ValueError as e: print(f"{Fore.RED}Invalid ({e})."); continue

        for idx in selected_indices:
            item  = all_summaries[idx]; entry = item["entry"]
            prev  = entry["Error"][:60]+("..." if len(entry["Error"])>60 else "")
            print(f"\n{Fore.CYAN}[{idx+1}] Analysing: {prev}")
            txt = analyze_issue(entry, item["cat_name"])
            result = {"ComputerName":entry["ComputerName"],"Category":item["cat_name"],
                      "Date":entry["Date"],"FileNames":", ".join(entry["FileNames"]),
                      "Error":entry["Error"],"Count":entry["Count"],"Analysis":txt}
            ex = next((a for a in all_analyses if a["Error"]==entry["Error"]
                       and a["ComputerName"]==entry["ComputerName"]), None)
            if ex: ex["Analysis"] = txt
            else:  all_analyses.append(result)
            csv_analysis_map.setdefault(OUTPUT_CSV, [])
            ex2 = next((a for a in csv_analysis_map[OUTPUT_CSV]
                        if a["Error"]==entry["Error"]), None)
            if ex2: ex2["Analysis"] = txt
            else:   csv_analysis_map[OUTPUT_CSV].append(result)
            print(f"\n{Fore.GREEN}--- Analysis ---\n{txt}\n{Fore.GREEN}{'─'*60}")
    ...
    if all_analyses:
        _flush_report(all_summaries, all_analyses, csv_analysis_map,
                      keyword_hits, keyword_computers, scanned_at, folder_path)
...
def _flush_report(all_summaries, all_analyses, csv_analysis_map,
    ...
    generate_html_report(
        full, rp,
        scan_range_from   = folder_path,
        scan_range_to     = scanned_at,
        total_lines       = total,
        keyword_hits      = keyword_hits,
        keyword_computers = keyword_computers,
    )
...
# ══════════════════════════════════════════════════════════════════════════════
# LOCAL FILE WORKER  — scans one file, no date filter
# ══════════════════════════════════════════════════════════════════════════════
def _scan_file(fpath: str, fname: str, search_queries: list) -> list:
    """Scan a single local file for keyword matches."""
    raw = []
    try:
        ...
        for line in content.splitlines():
            ...
            for query in search_queries:
                if query in clean.lower():
                    raw.append({
                        "Computer":       fname,   # filename as "computer" label
                        "File":           fname,
                        "Content":        clean,
                    ...
                    break
        with _cache_lock:
            local_file_cache[("local", "LocalScan", fname)] = fpath
    except Exception as e:
        tprint(f"  {Fore.RED}Error reading {fname}: {e}")
    return raw


# ══════════════════════════════════════════════════════════════════════════════
# MAIN SCAN  — no FTP, no time range, reads local folder
# ══════════════════════════════════════════════════════════════════════════════
def scan_logs():
    # ── Load keywords ─────────────────────────────────────────────────────────
    if not os.path.exists(STRINGS_FILE):
        print(Fore.RED + f"Error: {STRINGS_FILE} not found!\n"
              "Create strings.txt with one keyword per line.")
        return [], {}, {}, "", ""
    ...
    # ── Find log files ────────────────────────────────────────────────────────
    folder_abs = os.path.abspath(LOG_FOLDER)
    if not os.path.isdir(folder_abs):
        print(Fore.RED + f"Error: LOG_FOLDER not found: {folder_abs}")
        return [], {}, {}, "", ""

    all_files = []
    for fname in sorted(os.listdir(folder_abs)):
        fpath = os.path.join(folder_abs, fname)
        if os.path.isfile(fpath) and should_scan(fname):
    ...
    print(f"\n{Fore.CYAN}{'='*60}")
    print(f"{Fore.CYAN}{Style.BRIGHT}  LOCAL SCAN")
    print(f"{'='*60}")
    print(f"{Fore.WHITE}Folder   : {Fore.YELLOW}{folder_abs}")
    print(f"{Fore.WHITE}Files    : {Fore.YELLOW}{len(all_files)}")
    if FILE_FILTER:
        print(f"{Fore.WHITE}Filter   : {Fore.YELLOW}{', '.join(FILE_FILTER)}")
    print(f"{Fore.WHITE}Keywords : {Fore.YELLOW}{len(search_queries)} → {', '.join(search_queries)}")
    print(f"{Fore.WHITE}KB cases : {Fore.YELLOW}{len(cases)}{Fore.WHITE} loaded")
    print(f"{Fore.CYAN}{'='*60}\n")

    if not all_files:
        print(f"{Fore.MAGENTA}No files found in {folder_abs}")
        if FILE_FILTER:
            print(f"{Fore.YELLOW}Hint: FILE_FILTER = {FILE_FILTER} — check if it matches your filenames")
        return [], {}, {}, scanned_at, folder_abs

    # ── Parallel file scanning ────────────────────────────────────────────────
    keyword_hits      = {q: 0 for q in search_queries}
    keyword_computers = {q: set() for q in search_queries}  # will hold filenames
    all_raw           = []

    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        futures = {pool.submit(_scan_file, fp, fn, search_queries): fn
                   for fp, fn in all_files}
        for future in as_completed(futures):
            fn = futures[future]
            try:
                file_raw = future.result()
                all_raw.extend(file_raw)
                hits = len(file_raw)
                tprint(f"  {Fore.GREEN if hits else Fore.BLUE}[{fn}] "
                       f"{hits} hit{'s' if hits!=1 else ''}")
            except Exception as e:
                tprint(f"  {Fore.RED}[{fn}] Error: {e}")
    ...
    for res in all_raw:
        ...
        if kw in keyword_hits:
            keyword_hits[kw]      += 1
            keyword_computers[kw].add(res["File"])

    # ── Group similar results (90%) ───────────────────────────────────────────
    final_summary = []
    for res in all_raw:
        matched = False
        for entry in final_summary:
            if (res["Computer"] == entry["ComputerName"]
                    and is_similar(res["Content"], entry["Error"])):
                entry["Count"] += 1
                if res["File"] not in entry["FileNames"]:
                    entry["FileNames"].append(res["File"])
                matched = True; break
        if not matched:
            final_summary.append({
                "ComputerName": res["File"],    # use filename as identifier
                "Date":         scanned_at[:10],
                "FileNames":    [res["File"]],
                "Error":        res["Content"],
                "Count":        1,
    ...
    final_summary.sort(key=lambda x: -x["Count"])

    # ── Write CSV ─────────────────────────────────────────────────────────────
    if final_summary:
        with open(OUTPUT_CSV, "w", newline="", encoding="utf-8-sig") as f:
            writer = csv.writer(f)
            writer.writerow(["FileName","Date","FileNames","Error","Count","AI_Analysis"])
            for s in final_summary:
                writer.writerow([s["ComputerName"], s["Date"],
                                  ", ".join(s["FileNames"]),
                                  s["Error"], s["Count"], ""])
        print(f"\n{Fore.GREEN}{Style.BRIGHT}CSV saved: {OUTPUT_CSV}")

    all_summaries = [{"entry": e, "cat_name": "LocalScan", "csv_path": OUTPUT_CSV}
                     for e in final_summary]
    return all_summaries, keyword_hits, keyword_computers, scanned_at, folder_abs
...
if __name__ == "__main__":
    result = scan_logs()
    all_summaries, keyword_hits, keyword_computers, scanned_at, folder_abs = result
    ...
    if all_summaries:
        ...
        if go == "y":
            interactive_analysis(all_summaries, keyword_hits, keyword_computers,
                                  scanned_at, folder_abs)
        else:
            _flush_report(all_summaries, [], {}, keyword_hits, keyword_computers,
                          scanned_at, folder_abs)
    else:
        print(f"{Fore.MAGENTA}No keyword hits found.")
        print(f"{Fore.WHITE}Check {Fore.YELLOW}strings.txt{Fore.WHITE} keywords "
              f"and {Fore.YELLOW}FILE_FILTER{Fore.WHITE} setting.")
